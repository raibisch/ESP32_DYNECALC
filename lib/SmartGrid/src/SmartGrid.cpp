...
{
  fs::File cFile = SPIFFS.open(filename, "w");
  if (!cFile) 
  {
...
bool SmartGrid::updateCost_monthday(uint8_t akt_day)
{
  ...
  for (size_t i = 1; i <= 31; i++)
  {
    ...
    smdfile += stmp;
    stmp = "";
    stmp.format_P("%.2f;", getFixprice_monthday(i));
    smdfile += stmp;
    stmp = "";
  ...
  // todo: not testet !!! 
  // anstelle von dem da 
  File cFile = SPIFFS.open("/cost_akt_month.txt", "w");
  if (!cFile) 
  {
...
bool SmartGrid::loadCost_month()
{
  ...
  File cFile = SPIFFS.open("/cost_akt_year.txt");
  if (!cFile) 
  {
...
bool SmartGrid::updateCost_month(uint8_t month)
{
  ...
  for (size_t i = 0; i <= 11; i++)
  {
    ...
    smdfile += stmp;
    stmp = "";
    stmp.format_P("%.2f;", getFixprice_month(i));
    smdfile += stmp;
    stmp = "";
...
void SmartGrid::loop(time_t* time_now)
{
    ...
    if (_bUpdate) // extra Bool for init at startup
    {
      ...
      delay(800);
      _bUpdate = !downloadWebData(time_now, true); // day-ahead hat nicht immer daten !!!
