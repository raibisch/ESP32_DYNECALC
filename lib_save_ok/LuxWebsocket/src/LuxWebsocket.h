...
#define debug_printluxval(...) Serial.printf("[LUX] %s:\t %s\r\n",__VA_ARGS__, __VA_ARGS__);
#else
#define debug_begin(...)
#define debug_print(...)
#define debug_printf(...)
...
/// LUXTRONIK DATA ***************************************
#define LUX_XMLDEF_GRAD_C       "°C"
#define LUX_XML_DEF_KWH         " kWh"
...
#define LUX_XMLDEF_TEMP_RL_IST  "cklauf</name><value>"
#define LUX_XMLDEF_TEMP_RL_SOLL "ckl.-Soll</name><value>"    // 10/2024
#define LUX_XMLDEF_TEMP_WW_IST  "Warmwasser-Ist</name><value>"
#define LUX_XMLDEF_TEMP_OUT     "Außentemperatur</name><value>"
#define LUX_XMLDEF_TEMP_HEGAS   "Heissgas</name><value>"
#define LUX_XMLDEF_TEMP_OUTM    "Mitteltemperatur</name><value>"
#define LUX_XMLDEF_TEMP_VDHEI   "VD-Heizung</name><value>"
#define LUX_XMLDEF_TEMP_WQ_IN   "rmequelle-Ein</name><value>"
#define LUX_XMLDEF_TEMP_VD_IN   "Ansaug VD</name><value>"

#define LUX_XMLDEF_ENERGY_OUT   "rmemenge"
//--------------------------------------------------
#ifdef Luxtronik_V3_89
#define LUX_XMLDEF_ENERGY_IN    "Eingesetzte Energie"
#else // >= V 3.90
#define LUX_XMLDEF_ENERGY_IN    "Leistungsaufnahme"
#endif
//-------------------------------------------------

// Eingänge ---------------------------------------
#define LUX_XML_DEF_FLOW        "Durchfluss</name><value>"

// Ausgänge ----------------------------------------

// Anlagenstatus -----------------------------------
#define LUX_XML_DEF_POWER_OUT "Heizleistung Ist</name><value>"
#ifndef Luxtronik_V3_89
#define LUX_XML_DEF_POWER_IN  "Leistungsaufnahme</name><value>"
#endif
#define LUX_XML_DEF_DEFROST_PERCENT "Abtaubedarf</name><value>"
#define LUX_XML_DEF_KWH_SUMIN   "Gesamt"
...
{
  private: 
  static WStype_t wstype;
  static LUX_HEATPUMP_STATUS hp_state;

  LUX_VAL_TYPE valtype;

  uint32_t energyElektIn;
  uint32_t energySumIn;
  uint32_t energyWWIn;
  uint32_t energyHEIn;

  uint32_t energySumOut;
  uint32_t energyWWOut;
  uint32_t energyHEOut;

  uint32_t energyOldElectIn =0;
  uint32_t energyOldSumIn   =0;
  uint32_t energyOldWWIn    =0;
  uint32_t energyOldHEIn    =0;
  
  uint32_t energyOldSumOut=0;
  uint32_t energyOldWWOut =0;
  uint32_t energyOldHEOut =0;
  ...
  protected:
     ...
     void calcEnergyValues(bool bDayOld);
     void calcCopDay();
  ...
  public:
    LuxWebsocket();
    ~LuxWebsocket ();
    static WS_POLL_STATUS poll_state;

    bool init(const char* url, const char* password);
    void poll();
    void setpolling(bool on_off);
    bool isConnected() {
                         if (poll_state >= WS_POLL_STATUS::SEND_GET_INFO)
                         {return true;}
                         return false;
                       }
    const char* getCSVfetch(bool all=false);
    const char* getval(LUX_VAL_TYPE valtype);
